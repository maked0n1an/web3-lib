...
from tasks.coredao.coredao_data import CoreDaoBridgeData
...
class CoreDaoBridge(BaseTask):
    async def bridge(
        ...
        src_bridge_data = CoreDaoBridgeData.get_token(
    ...
    async def _get_estimateBridgeFee(
        ...
        estimate_fee = await contract.functions.estimateBridgeFee(
            False,
            '0x'
        ).call()
        
        return TokenAmount(
            amount=estimate_fee[0],
